...
volatile sig_atomic_t received_flag = false;   //Ashutosh:  A boolean variable required to stop re-transmitting once ACK is received
...
/* Ashutosh: Interrupt handler to re-transmit the packet */
void handle_alarm( int sig ) {

if (received_flag == false)   // don't re-transmit of ACK received
{
	
	printf("Retransmission time");
...
int main(void)
{
	...
	signal( SIGALRM, handle_alarm ); // Ashutosh: Re-transmission Handler
 ...
 int state = 0;
	...
	{ case 0:      printf("Enter message 0: ");//wait for sending packet with seq. no. 0
			...
			state = 1;
			received_flag = false; //Ashutosh
 		    alarm( 1 ); // Ashutosh: Run the timer
 			break;
	  ...
	  case 1:    //wait for ACK 0
		    ...
		    if (rcv_ack.sq_no==0)
                     {  
                        ...
                        state = 2; 
						received_flag = true;  // Ashutosh: Message received, stop re-transmiting
			     break;
                     }
          ...
          case 3:	//waiting for ACK 1
			...
			if (rcv_ack.sq_no==1)
                     	  {  
							  printf("Received ack seq. no. %d\n",rcv_ack.sq_no);
                             state = 0; 
							 received_flag = true; //Ashutosh: message received, stop re-transmitting
			    			 break;
               	           }
