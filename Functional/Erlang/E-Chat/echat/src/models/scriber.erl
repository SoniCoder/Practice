...
-export([create_or_update/3, delete/1, delete_all/0,
	 get_all/0, get_all/1, get_by_id/1, get_by_id/2,
	 get_column_by_id/2, get_column_by_id/3, search_by/2,
	 update/4, update_columns_by_id/2]).

%-spec create_or_update(seat_name(), pps_seat()|undefined, metadata()) ->
%                               {ok, Key :: key()} | {error, term()}.
create_or_update(_TimeStamp, Scriber, _Metadata) ->
    F = fun () -> mnesia:write(Scriber) end,
...
delete_all() -> baseinfo:delete_all(?MODULE).

%-spec delete(seat_name())->ok|{error,term()}.
delete(SeatName) -> baseinfo:delete(?MODULE, SeatName).

-spec update(key(), {module(), atom(), [term()]},
	     [{no_create, true | false} |
	      {pass_metadata, true | false}],
	     [term()]) -> ok | {ok, key()} | any().

update(Key, {Module, Function, Args}, Options,
       _UpdateOptions) ->
...
get_all() ->
    F = fun () ->
		mnesia:select(?MODULE, [{'_', [], ['$_']}])
	end,
    mnesia:transaction(F).

    %baseinfo:get_all(?MODULE).

-spec get_all(db_columnlist() | key) -> {ok, list()} |
					{error, term()}.
...
get_column_by_id(SeatName, ColumnName) ->
    baseinfo:get_column_by_id(?MODULE, SeatName,
			      ColumnName).
...
-spec search_by(db_whereclauselist(),
		db_columnlist() | record | key) -> {ok, list()} |
						   {error, term()}.

search_by(WhereClauseList, ColumnList) ->
    baseinfo:search_by(?MODULE, WhereClauseList,
		       ColumnList).

-spec update_columns_by_id(key(),
			   [{atom(), term()}]) -> {ok, key()} | {error, term()}.
